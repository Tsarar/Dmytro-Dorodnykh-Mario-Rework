#include "../headers/DeathMenu.h"
	...
	DeathMenu::DeathMenu() {
		yesButton = new sf::Texture;				//dynamic memory usage
		noButton = new sf::Texture;				//
		textTexture = new sf::Texture;				//
		yes = new sf::Sprite;					//
		no = new sf::Sprite;					//
		text = new sf::Sprite;					//
	...
	DeathMenu::~DeathMenu() {
		delete yesButton;		//freeing memory
		delete noButton; 		//
		delete textTexture;		//
		delete yes;			//
		delete no;			//
	...
	bool DeathMenu::run(sf::RenderWindow &window) {
		while(window.isOpen()) {	//main cycle
					...
					*sizeVector = window.getSize();		//if somebody will decide to scretch
					...
					window.draw(*yes);			//display everything
					window.draw(*no);			//
					window.draw(*text);			//
										//
					window.display();			//
