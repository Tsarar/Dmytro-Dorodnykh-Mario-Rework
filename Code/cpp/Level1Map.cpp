#include "../headers/Level1Map.h"
	...
	Level1Map::Level1Map(sf::RenderWindow &window, bool showMenu) : LevelMap(window) {
		...
		*coinsToWin=11;				//
		...
		goDeep = new bool;			//are we going deeper?
		*goDeep=false;				//
			 ...
			 {							
				if (Mario->alive())					//
					if (mushroom->getLife()) {			//if enemy alive
						 if (Mario->getDy()>0) { mushroom->setDx(0); Mario->setDy(-0.2); smashSound->play(); mushroom->setLife(false);}	//mario can smash him or
	...
	bool Level1Map::isEnemiesAlive() const
		{ if (mushroom->getLife()) return true; else return false; }
