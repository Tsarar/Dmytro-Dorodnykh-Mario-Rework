#include "../headers/Level2Map.h"
	...
	inline void Level2Map::drawAll() {
			...
			window.draw(slime1->getSprite());						//draw slime1
			window.draw(slime2->getSprite());						//draw slime1
			window.draw(slime3->getSprite());						//draw slime1
	...
	int Level2Map::update() {
		while (window.isOpen())	{
			...
			if (!(*pause)) {
					Mario->update(time);		//update our characters
					slime1->update(time);		//
					slime2->update(time);		//
					slime3->update(time);		//
				};
			...
			if (!(Mario->alive())) playerDeath(time);
	...
	inline void Level2Map::enemyCollision(Entity &slime) {
			if (Mario->alive())					//
				if (slime.getLife()) {			//if enemy alive
					if (Mario->getDy()>0) { slime.setDx(0); Mario->setDy(-0.2); Mario->hurt(); smashSound->play(); slime.setLife(false);}	//mario can smash it and take damage or
	...
	bool Level2Map::isEnemiesAlive() const 
		 { if (slime1->getLife() || slime2->getLife() || slime3->getLife()) return true; else return false; };