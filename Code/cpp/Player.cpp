#include "../headers/Player.h"
		...
		Player::Player(LevelMap &_map) : map(_map) {	//setting connection between level and player
			...
			isOnGround = new bool;			//
			isTeleported = new bool;		//
			isBlinking = new bool;			//
			damageTicks = new int;			//
			damageClock = new sf::Clock;		//
			healingClock = new sf::Clock;		//
			platformClock = new sf::Clock;		//
			timeTick = new float;			//
			lives = new int;			//
			livesTexture = new sf::Texture;		//
			hearts = new sf::Sprite;		//
			coins = new int;			//
			isHealed = new bool;			//
			isWon = new bool;			//
			currentLevel = new int;			//
			isDownThePlatform = new bool;		//
			winCondition = new int;			//
			isOnLadder = new bool;			//
			...
			hitBuff = new sf::SoundBuffer;				//sound buffers
			coinBuff = new sf::SoundBuffer;				//
			teleportBuff = new sf::SoundBuffer;			//
			unicornBuff = new sf::SoundBuffer;			//
			bonusBuff = new sf::SoundBuffer;			//
			deathBuff = new sf::SoundBuffer;			//
			...
			hitSound = new sf::Sound;				//sounds
			coinSound = new sf::Sound;				//
			teleportSound = new sf::Sound;				//
			unicornSound = new sf::Sound;				//
			bonusSound = new sf::Sound;				//
			deathSound = new sf::Sound;				//
		...
		Player::~Player() {
			delete currentFrame;	//freeing memory
			delete sprite;		//
			delete rect;		//
			delete dy;		//
			delete dx;		//
		...
		void Player::update(float time)	{	//main procedure	
			...
			if (!(*isOnGround) && !(*isOnLadder)) *dy=*dy+float(0.0005)*time;	//gravitation
			rect->top += *dy*time;							//getting higher or lower
			*isOnGround=false;							//for different interactions
			*isOnLadder=false;							//
			collision(1);								//y coord blocks interation

			*currentFrame = *currentFrame + time * float(0.005);	//updating frame without connection to the processor frequency
			...
			if (*isBlinking) {							//blinking
					*timeTick=damageClock->getElapsedTime().asSeconds();	//for a tick delay	
					if (*timeTick>0.35) {					//can be easily changed
							...
							*damageTicks=*damageTicks+1;																			//counter++
		...
		void Player::hurt()	{ 			//if player hurts
			if (!(*isBlinking)) {
					*isBlinking=true;	//blinking
					hitSound->play();	//sound
					*lives = *lives - 1;	//hurting
		...
		inline void Player::spriteUpdate() {	//sprite change
			if (*dx>0) (*dy==0) ? sprite->setTextureRect(sf::IntRect(112+16*int(*currentFrame),144,16,16)) : sprite->setTextureRect(sf::IntRect(162,144,16,16));		//running or jumping to the left
			if (*dx<0) (*dy==0) ? sprite->setTextureRect(sf::IntRect(112+16*int(*currentFrame)+16,144,-16,16)) : sprite->setTextureRect(sf::IntRect(178,144,-16,16));	//or right
			if (*dx==0) (*dy==0) ?  sprite->setTextureRect(sf::IntRect(97,144,16,16)) : sprite->setTextureRect(sf::IntRect(162,144,16,16));					//standing or falling
		}
		...
		void Player::collisionCheckSolidBonus (int iCoord,int jCoord, int mode) {
				if (*dy>0 && mode==1) { rect->top = float(iCoord*16 -  rect->height); *dy=0; *isOnGround=true; }			//if we hit the floor from above we are on ground
				if (*dy<0 && mode==1) { rect->top = float(iCoord*16 + 16); *dy=0; bonusSound->play(); map.setTileMap(iCoord,jCoord,'e'); map.setTileMap(iCoord-1,jCoord,'C'); } //if we hit bonus from bottom it spawns coin
				if (*dx>0 && mode==0)   rect->left = float(jCoord*16 - rect->width);							//if we hit solid block from the side
				if (*dx<0 && mode==0)   rect->left = float(jCoord*16 + 16);								//we won't get through it
			}
		...
		inline void Player::collisionCheckPlatform(int iCoord,int jCoord, int mode) {
			if (*dy>0 && mode==1)  if (!(*isDownThePlatform)) { rect->top = float(iCoord*16 -  rect->height); *dy=0; *isOnGround=true; if (sf::Keyboard::isKeyPressed(sf::Keyboard::Down)) { *isDownThePlatform=true; platformClock->restart(); } }
